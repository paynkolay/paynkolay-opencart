	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($opencartVersion == "2.3"){
                $this->response->redirect($this->url->link('extension/payment/nkolaypos', 'token=' . $this->session->data['token'], 'SSL'));
            }
            else{
                $this->response->redirect($this->url->link('payment/nkolaypos', 'token=' . $this->session->data['token'], 'SSL'));
            }
		...
		if($opencartVersion == "2.3"){
            $data['action'] = $this->url->link('extension/payment/nkolaypos', 'token=' . $this->session->data['token'], 'SSL');
        }
        else{
            $data['action'] = $this->url->link('payment/nkolaypos', 'token=' . $this->session->data['token'], 'SSL');
        }
