...
class ControllerExtensionPaymentNkolayPos extends Controller
{
    public function index()
    {
        ...
        $callbackUrl = $this->url->link('extension/payment/nkolaypos/callback', '', true);

        $formData = $client->buildRedirectFormData([
            'clientRefCode' => PayNKolayClient::buildClientRefCode('Opencart3x', $this->session->data['order_id']),
            'amount'        => $amount,
            'currencyCode'  => $currencyNumber,
            'successUrl'    => $callbackUrl,
            'failUrl'       => $callbackUrl,
            'use3D'         => $this->config->get('payment_nkolaypos_type') == '3D' ? 'true' : 'false',
            'platform'      => PayNKolayClient::platformTag('Opencart3x', VERSION),
        ]);
    ...
    private function confirmOrder(string $orderId): void
    {
        ...
        $this->model_checkout_order->addOrderHistory(
            $orderId,
            $this->config->get('payment_nkolaypos_order_status_id')
        );
    }
