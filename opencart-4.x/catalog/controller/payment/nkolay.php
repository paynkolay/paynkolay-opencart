...
if (!defined("VERSION")) exit;
    class Nkolay extends \Opencart\System\Engine\Controller
    {
        ...
        {
            if (!$this->config->get('payment_nkolaypos_sx')) {
                return "Payment already configured";
            }
            ...
            $callBack           = $this->url->link('extension/nkolay/payment/nkolay' . $this->separator . 'callback');
            $failBack           = $this->url->link('extension/nkolay/payment/nkolay' . $this->separator . 'callback');
            $sx                 = $this->config->get('payment_nkolaypos_sx');
            $merchantSecretKey  = $this->config->get('payment_nkolaypos_secret');
            $successUrl         = $callBack;
            $failUrl            = $failBack;
            $amount             = $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false);
            $clientRefCode      = 'Opencartv4|'.$this->session->data['order_id'] . "|OP|" . uniqid();
            ...
            $data['data'] = [
                ...
                'hashData'      => $hashData,
                'ECOMM_PLATFORM'=> 'Opencartv4'
            ];
          
            $base_url = explode("index.php", $_SERVER['SCRIPT_URI'])[0];
        ...
        public function callback()
        {
            ...
            $order_id               = str_replace('Opencartv4|','', current($order_id));
            ...
            if ($this->request->post['USE_3D'] == 'false' && $this->request->post['hashData'] == $hashDatauretilen) {
                ...
                if ($this->request->post['RESPONSE_CODE'] == 2 && $this->request->post['hashData'] == $hashDatauretilen) {
                    ...
                    $this->model_checkout_order->addHistory($order_id, $this->config->get('payment_nkolaypos_order_status_id'));
